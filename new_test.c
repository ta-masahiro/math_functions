...
int main(int argc, char * argv[]) {
    ...
    mpfr_init(theta);mpfr_init(ans);mpfr_init(er1);mpfr_init(er2);mpfr_init(er3); mpfr_init(ser1);mpfr_init(ser2);mpfr_init(ser3);
    char buf[1024];
    ...
    for(i = 0; i<=90*n; i ++ ) {
        ...
        //th=(double)j;
        mpfr_const_pi(theta, MPFR_RNDN); mpfr_mul_si(theta, theta, (long)i, MPFR_RNDN); mpfr_div_si(theta, theta, 180*n, MPFR_RNDN); 
    ...
    printf("\t%f\t%f\t%f\n", mpfr_get_d(ser1,MPFR_RNDN), mpfr_get_d(ser2,MPFR_RNDN), mpfr_get_d(ser3,MPFR_RNDN)); 
}
